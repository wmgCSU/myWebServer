...
sylar::ConfigVar<std::map<std::string,int>>::ptr g_str_int_map_value_config =
	sylar::Config::Lookup("system.str_int_map", std::map<std::string, int>{ {"key",1 }}, "system str_int_map");
sylar::ConfigVar<std::unordered_map<std::string, int>>::ptr g_str_int_umap_value_config =
	sylar::Config::Lookup("system.str_int_umap", std::unordered_map<std::string, int>{ {"key", 1 }}, "system str_int_umap");
...
void test_yaml() {
	YAML::Node root = YAML::LoadFile("testyaml.yml");
	print_yaml(root, 0);
...
void test_config() {
	...
	//SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_int_valuex_config->getValue();
...
public:
	...
	bool operator == (const Person& oth) const {
		if (m_name == oth.m_name && m_age == oth.m_age && m_sex == oth.m_sex) return true;
		return false;
	}
};
namespace sylar {
...
sylar::ConfigVar<std::map<std::string, Person> >::ptr g_map =
	sylar::Config::Lookup("class.map", std::map<std::string, Person>(), "system person map");
sylar::ConfigVar<std::map<std::string, std::vector<Person> > >::ptr g_vec_map =
...
#define XX_PM(g_var,prefix) { \
	...
	XX_PM(g_map, before);
	SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_vec_map->toString();
	...
	XX_PM(g_map, after);
	SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "after: " << g_vec_map->toString();
};
#endif

void test_log() {
	YAML::Node root = YAML::LoadFile("logs.yml");
	sylar::Config::LoadFromYaml(root);
}
